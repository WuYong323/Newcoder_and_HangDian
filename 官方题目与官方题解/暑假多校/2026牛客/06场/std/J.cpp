...
int main() {
    ...
    while (n % B) n++, a.push_back(0), b.push_back(1);
    ...
    std::vector<std::pair<Frac, int>> maxf(n / B);
    std::vector<Convex> hull(n / B);

    auto build = [&](int i) -> void {
        maxf[i] = {Frac(0, 1), -1};
        for (int j = i * B; j < (i + 1) * B; j++) {
            maxf[i] = std::max(maxf[i], std::make_pair(Frac(a[j], b[j]), j));
            f[j] = Frac(a[j], b[j]);
            p[j] = Point(b[j], a[j]);
        }
        hull[i].init(std::vector(p.begin() + i * B, p.begin() + (i + 1) * B));
    };
    ...
    while (q--) {
        ...
        if (o == 1) {
            ...
            l--;
            r--;
            std::pair<Frac, int> max = {Frac(0, 1), -1};
            for (int i = id[l]; i <= id[r]; i++) {
                int lb = i * B, rb = (i + 1) * B - 1;
                if (l <= lb && r >= rb) {
                    max = std::max(max, maxf[i]);
                } else {
                    for (int j = std::max(l, lb); j <= std::min(r, rb); j++) {
                        max = std::max(max, std::make_pair(f[j], j));
                    }
                }
            }
            Frac ans(0, 1);
            for (int i = id[l]; i <= id[r]; i++) {
                int lb = i * B, rb = (i + 1) * B - 1;
                if (l <= lb && r >= rb && (max.second < lb || max.second > rb)) {
                    int left = 0, right = hull[i].up - 2;
                    while (left <= right) {
                        int mid = (left + right) / 2;                        
                        Point p1 = hull[i].upper(hull[i].up - 1 - mid);
                        Point p2 = hull[i].upper(hull[i].up - 1 - (mid + 1));                        
                        if (toLeft(p2, Line(Point(-max.first.den, -max.first.num), p1)) > 0) {
                            left = mid + 1;
                        } else {
                            right = mid - 1;
                        }
                    }
                    auto [x, y] = hull[i].upper(hull[i].up - 1 - left);
                    ans = std::max(ans, max.first + Frac(y, x));
                } else {
                    for (int j = std::max(l, lb); j <= std::min(r, rb); j++) if (j != max.second) {
                        ans = std::max(ans, max.first + f[j]);
                    }
                }
            }
            auto [x, y] = ans;
